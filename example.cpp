...
// PC用のテスト用の関数です。
inline int NesoraWave(std::string filename, std::vector<int16_t> wave) {
	std::ofstream file(filename, std::ios::out | std::ios::binary);
	if (!file) return 1;
	...
	file.write("fmt ", 4);											//fmt
	file.write((char*)&(work = 16), 4);								//size
	file.write((char*)&(work = 1), 2);								//format(32bit float)
	file.write((char*)&(work = 1), 2);								//channel(monaural)
	file.write((char*)&(work = smpl), 4);								//sampling frequency
...
int test(){
	...
	MVFF.textread("<s50><bf440><p261>ka<p293>e<p329>ru<p349>no<p329>u<p293>ta<p261>ga");
...
int main() {
	...
	// UTAU音源を出力する関数です。
	// macの方用。
	// 文字コードの関係があるので、ゴリ押しでShift-JISを生成しています。
	ToUTAU(MVFF, "音諳 風鈴25");
...
// このプログラムをちょっといじってUTAU音源を生成できるようにしたものです。
// ついでにmacで動くようになってます。
// clangでコンパイルしてやってください。
// windowsで動くかは知りません。
// 一応コンパイラ依存のものは使ってないと思うので、g++とか使えば動くと思います。
// 
// そんな感じ
// 
